...
assert.equal(glyph.status,'SOURCE_LOCKED_AWAITING_EVIDENCE');
assert.equal(glyph.glyphs.length,0);
assert.equal(glyph.phoneticUnits.length,0);
assert.equal(glyph.compositionRules.length,0);
assert.ok(glyph.unresolved.length>=5);

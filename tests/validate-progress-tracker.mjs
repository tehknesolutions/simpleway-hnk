...
const snap=await json('progress/CYCLE1_PROGRESS_SNAPSHOT_V126.json');
...
assert.equal(snap.snapshot_id,'SWHNK-C1-PROGRESS-SNAPSHOT-V126');
...
assert.equal(snap.language_architecture.KE_binding_proposal_status,'APPROVED_ALL_PROPOSAL_AND_GOVERNANCE_DECISIONS');
assert.equal(snap.language_architecture.current_chat_active_gate,appGate.batch_id);
...
assert.equal(snap.parallel_workstreams.parent_lexeme_canonical_integration.consumed_by_v126,false);
assert.equal(snap.next_gate,appGate.batch_id);
...
console.log('PASS SWHNK-C1-PROGRESS-TRACKER-V126');

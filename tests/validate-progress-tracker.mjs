...
const qaTransition=await json('curriculum/cycle-01/L01-kether/validation/qa.validated-transition.v1.json');
...
assert.equal(values.filter(x=>x.implementation_state==='VALIDATED').length,96);
assert.equal(values.filter(x=>['AUTHORED','VALIDATED','FROZEN'].includes(x.implementation_state)).length,101);
assert.equal(values.filter(x=>x.implementation_state==='FROZEN').length,0);
assert.equal(values.filter(x=>x.scaffolded).length,101);
...
assert.equal(qaBatch.status,'APPROVED_AND_APPLIED_SCOPED_COURSE_VALIDATION');
assert.ok(qaBatch.decisions.every(x=>x.decision==='APPROVED_SCOPED'));
assert.equal(qaTransition.status,'APPLIED');
assert.equal(qaTransition.global_after.VALIDATED,96);
...
assert.equal(storyBatch.projected_effect_if_all_approved.global_VALIDATED_after,101);
...
console.log('PASS SWHNK-C1-PROGRESS-TRACKER-V23');
console.log('Cycle 1: 5 AUTHORED + 96 VALIDATED = 101 authored-or-better; L01 OPI, Activation, structures, teacher notes and Q&A fully validated; 5 Stories authored, validation pending.');

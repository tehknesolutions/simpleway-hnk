...
assert.deepEqual(bindings.metrics.validated_opi, ['L01-OPI-001','L01-OPI-002','L01-OPI-003','L01-OPI-004','L01-OPI-005','L01-OPI-006','L01-OPI-007','L01-OPI-008','L01-OPI-009','L01-OPI-010']);
...
assert.equal(opi7.validation_state, 'VALIDATED_FOR_L01_V1_1_COURSE_USE_WITH_VANI_MEANING_NULL_WATCH_KUON_CANDIDATE_AND_ON_GATE_VISIBLE');
assert.deepEqual(opi7.hnk_question_sequence, ['EN VANI KUVAN KE','EN VANI KUON KE']);
assert.deepEqual(opi7.experimental_recovered_form_authority, ['WATCH']);
assert.deepEqual(opi7.experimental_recovered_form_meaning, [null]);
assert.equal(opi7.semantic_hypothesis.scope, 'SIMPLEWAY_HNK_L01_OPI_007_ONLY');
assert.equal(opi7.semantic_hypothesis.historical_gloss_recovered, false);
assert.deepEqual(opi7.authored_candidate_ids, ['AUTH-001','AUTH-003']);
...
assert.equal(evidence.validation_evidence.L01_OPI_authored_current_state, 0);
assert.equal(evidence.validation_evidence.L01_OPI_validated, 10);
assert.deepEqual(evidence.validation_evidence.L01_OPI_hold_slots, []);

console.log('PASS SWHNK-C1-PROGRESS-TRACKER-V10');
console.log('1008 slots locked; L01 OPI is 10/10 VALIDATED for scoped v1.1 course use; 0 FROZEN; VANI meaning=null, WATCH/CANDIDATE/GATE boundaries and 45 governed language assets remain unchanged.');

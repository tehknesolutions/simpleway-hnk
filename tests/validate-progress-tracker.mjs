...
const snap=await json('progress/CYCLE1_PROGRESS_SNAPSHOT_V133.json');
...
assert.equal(snap.snapshot_id,'SWHNK-C1-PROGRESS-SNAPSHOT-V133');
...
assert.equal(snap.L03.first_payload_candidate_route,'DEDICATED_MINIMAL_CLAUSE_PAYLOAD_CANDIDATE_FIRST');
assert.equal(snap.L03.first_payload_candidate_proposal_approved,true);
...
assert.equal(snap.L03.first_payload_surface_function_completed_fields,0);
assert.equal(snap.L03.first_payload_surface_function_package_complete,false);
...
assert.equal(snap.language_architecture.first_payload_candidate_proposal_status,'APPROVED_ALL_PROPOSAL_AND_GOVERNANCE_DECISIONS');
assert.equal(snap.language_architecture.current_chat_active_gate,exactGate.batch_id);
assert.equal(snap.language_architecture.current_chat_active_gate_status,'AWAITING_EXPLICIT_HUMAN_SURFACE_FUNCTION_PACKAGE');
...
assert.equal(snap.boundaries.first_payload_candidate_proposal_approved,true);
...
assert.equal(snap.parallel_workstreams.parent_lexeme_canonical_integration.consumed_by_v133,false);
assert.equal(snap.next_gate,exactGate.batch_id);
assert.equal(snap.next_gate_status,'AWAITING_EXPLICIT_HUMAN_SURFACE_FUNCTION_PACKAGE');
...
console.log('PASS SWHNK-C1-PROGRESS-TRACKER-V133');

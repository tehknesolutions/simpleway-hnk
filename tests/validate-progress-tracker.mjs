...
const snap=await json('progress/CYCLE1_PROGRESS_SNAPSHOT_V127.json');
...
assert.equal(snap.snapshot_id,'SWHNK-C1-PROGRESS-SNAPSHOT-V127');
...
assert.equal(snap.language_architecture.KE_binding_application_status,'APPROVED_ALL_APPLICATION_AND_GOVERNANCE_DECISIONS');
...
assert.equal(snap.language_architecture.current_chat_active_gate,exactGate.batch_id);
assert.equal(snap.language_architecture.current_chat_active_gate_status,'AWAITING_EXPLICIT_HUMAN_SURFACE_PACKAGE');
...
assert.equal(snap.parallel_workstreams.parent_lexeme_canonical_integration.consumed_by_v127,false);
assert.equal(snap.next_gate,exactGate.batch_id);
...
console.log('PASS SWHNK-C1-PROGRESS-TRACKER-V127');

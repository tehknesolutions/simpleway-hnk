...
const snap=await json('progress/CYCLE1_PROGRESS_SNAPSHOT_V138.json');
...
assert.equal(snap.snapshot_id,'SWHNK-C1-PROGRESS-SNAPSHOT-V138');
...
assert.equal(snap.L03.first_payload_candidate_status,'VALIDATED_ALLOWLISTED_SCOPED_PRODUCTIVE_NONRUNTIME');
...
assert.deepEqual(snap.L03.first_payload_productive_realizations,['VALI KE','PARAZAMI KE']);
...
assert.equal(snap.language_architecture.first_payload_productivity_promotion_status,'APPROVED_ALL_SCOPED_PRODUCTIVITY_AND_GOVERNANCE_DECISIONS');
...
assert.equal(snap.parallel_workstreams.parent_lexeme_canonical_integration.consumed_by_v138,false);
...
console.log('PASS SWHNK-C1-PROGRESS-TRACKER-V138');

...
const snap=await json('progress/CYCLE1_PROGRESS_SNAPSHOT_V144.json');
...
assert.equal(snap.snapshot_id,'SWHNK-C1-PROGRESS-SNAPSHOT-V144');
...
assert.equal(snap.L03.OPI_first_position_package_proposal_approved,true);
...
assert.equal(snap.L03.OPI_slot_consumption,0);
...
assert.equal(snap.language_architecture.OPI_first_position_allocation_proposal_status,'APPROVED_ALL_PROPOSAL_AND_GOVERNANCE_DECISIONS');
assert.equal(snap.language_architecture.current_chat_active_gate,applicationGate.batch_id);
...
assert.equal(snap.boundaries.OPI_slot_consumption,0);
...
assert.equal(snap.parallel_workstreams.parent_lexeme_canonical_integration.consumed_by_v144,false);
assert.equal(snap.next_gate,applicationGate.batch_id);
...
console.log('PASS SWHNK-C1-PROGRESS-TRACKER-V144');

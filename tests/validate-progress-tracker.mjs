...
const snap=await json('progress/CYCLE1_PROGRESS_SNAPSHOT_V136.json');
...
assert.equal(snap.snapshot_id,'SWHNK-C1-PROGRESS-SNAPSHOT-V136');
...
assert.equal(snap.L03.controlled_clause_payload_authorized,true);
assert.equal(snap.L03.controlled_clause_authorized_payload_count,1);
assert.equal(snap.L03.slot_content_authorized,true);
assert.equal(snap.L03.first_payload_candidate_status,'VALIDATED_ALLOWLISTED_NONPRODUCTIVE_NONRUNTIME');
assert.equal(snap.L03.first_payload_candidate_validated,true);
assert.equal(snap.L03.first_payload_authorized,true);
assert.equal(snap.L03.first_payload_productivity_eligible,false);
...
assert.equal(snap.language_architecture.first_payload_validation_admission_status,'APPROVED_ALL_VALIDATION_ADMISSION_AND_GOVERNANCE_DECISIONS');
assert.equal(snap.language_architecture.current_chat_active_gate,eligibilityGate.batch_id);
assert.equal(snap.language_architecture.current_chat_active_gate_status,'AWAITING_EXPLICIT_HUMAN_APPROVAL');
assert.equal(snap.boundaries.controlled_clause_payload_authorized,true);
assert.equal(snap.boundaries.controlled_clause_authorized_payload_count,1);
assert.equal(snap.boundaries.first_payload_candidate_validated,true);
assert.equal(snap.boundaries.first_payload_authorized,true);
assert.equal(snap.boundaries.first_payload_productivity_eligible,false);
...
assert.equal(snap.parallel_workstreams.parent_lexeme_canonical_integration.consumed_by_v136,false);
assert.equal(snap.next_gate,eligibilityGate.batch_id);
...
console.log('PASS SWHNK-C1-PROGRESS-TRACKER-V136');

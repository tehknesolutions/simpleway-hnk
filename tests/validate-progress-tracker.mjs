...
const snap=await json('progress/CYCLE1_PROGRESS_SNAPSHOT_V139.json');
...
assert.equal(snap.snapshot_id,'SWHNK-C1-PROGRESS-SNAPSHOT-V139');
...
assert.equal(snap.L03.first_payload_curriculum_mapping_strategy_selected,true);
assert.equal(snap.L03.first_payload_selected_curriculum_mapping_route,'MAP_TO_L03_OPI_FOUNDATIONAL_INTERROGATIVE_FIRST');
...
assert.equal(snap.language_architecture.first_payload_curriculum_mapping_strategy_status,'APPROVED_ALL_STRATEGY_AND_GOVERNANCE_DECISIONS');
assert.equal(snap.language_architecture.current_chat_active_gate,mappingGate.batch_id);
assert.equal(snap.boundaries.first_payload_candidate_productive,true);
assert.equal(snap.boundaries.first_payload_curriculum_mapping_strategy_selected,true);
...
assert.equal(snap.parallel_workstreams.parent_lexeme_canonical_integration.consumed_by_v139,false);
assert.equal(snap.next_gate,mappingGate.batch_id);
...
console.log('PASS SWHNK-C1-PROGRESS-TRACKER-V139');
